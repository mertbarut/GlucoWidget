...
class WelcomeViewController: SplashScreenViewController, SplashScreenViewControllerDelegate {
    ...
    func requestHealthAuthorization() {
        ...
        healthStore.requestAuthorization(toShare: shareTypes, read: readTypes) { (success, error) in
            var status: String = ""
            //var userPreferredBloodGlucuseUnit: HKUnit?
            let quantityTypes : Set = [HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.bloodGlucose)!]
            ...
            if let error = error {
                ...
                if success {
                    ...
                    self.healthStore.preferredUnits(for: quantityTypes) { (preferredType, error) in
                        print("\(String(describing: preferredType.first?.value.unitString))")
                    }
                } else {
